...
def is_valid_desire_address(address, network='mainnet'):
    ...
    desire_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Desire address
        return False

    if (address_version != desire_version):
        return False
...
# shims 'til we can fix the desired side
def SHIM_serialise_for_desired(sentinel_hex):
    ...
    # superblock => "trigger" in desired
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # desired expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    desired_hex = serialise(obj)
    return desired_hex


# shims 'til we can fix the desired side
def SHIM_deserialise_from_desired(desired_hex):
    from models import DESIRED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(desired_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('desire.conf')
