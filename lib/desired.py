...
class DesireDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some desired methods
    ...
    @classmethod
    def from_desire_conf(self, desire_dot_conf):
        from desire_config import DesireConfig
        config_text = DesireConfig.slurp_config_file(desire_dot_conf)
        creds = DesireConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = desirelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = desirelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = desirelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_desired_proto_version_with_sentinel_ping)
